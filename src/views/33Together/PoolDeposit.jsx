...
export const PoolDeposit = props => {
  ...
  useEffect(() => {
    props.setInfoTooltipMessage([
      "Deposit sPSI to win! Once deposited, you will receive a corresponding amount of 33T and be entered to win until your sPSI is withdrawn.",
    ]);
  }, []);
          ...
          <Box display="flex" alignItems="center" flexDirection={`${isMobileScreen ? "column" : "row"}`}>
            ...
            {address && hasAllowance("sohm") ? (
              <Button
                ...
                fullWidth
              >
                {txnButtonText(pendingTransactions, "deposit", "Deposit sPSI")}
              </Button>
            ) : (
              <Button
                className="pool-deposit-button"
          ...
          {newOdds > 0 && quantity > 0 && (
            <Box padding={1}>
              <Typography variant="body2" style={{ color: "#33BB33" }}>
                Depositing {quantity} sPSI will increase odds of winning to 1 in {newOdds}&nbsp;
              </Typography>
            </Box>
          )}
          {/* NOTE (Appleseed): added this bc I kept losing track of which accounts I had sPSI in during testing */}
